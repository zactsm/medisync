...
export default function CaregiverSync({ user, patient, connectedCaregivers, sharedLog }) {
    ...
    return (
        <AppLayout user={user}>
            <Head title="Caregiver & Family Sync" />
            ...
            <div className="mb-8 p-6 rounded-2xl bg-gradient-to-r from-teal-950 via-slate-900 to-indigo-950 border border-teal-500/30 shadow-2xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                <div>
                    <div className="flex items-center gap-2 mb-2">
                        <Badge variant="teal">Caregiver Network Connected</Badge>
                        <span className="text-xs text-slate-400 font-mono">ID: {patient.syncCode}</span>
                    </div>
                    <h1 className="text-2xl font-black text-white">Sambungan Penjaga Keluarga (Caregiver Sync)</h1>
                    <p className="text-xs text-slate-300 mt-1 max-w-xl leading-relaxed">
                        Anak-anak dan ahli keluarga terdekat boleh memantau jadual ubat, temujanji hospital, serta menerima amaran langsung sekiranya ubat terlupa diambil.
                    </p>
                </div>

                {/* Pairing Code Card */}
                <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-700/80 text-center shrink-0 w-full md:w-auto">
                    <span className="text-[10px] uppercase font-bold text-slate-400 tracking-wider">Kod Sambungan Keluarga</span>
                    ...
                    >
                        ...
                        {copied ? 'Kod Disalin!' : 'Salin Kod Pasangan'}
                    </button>
                </div>
            </div>
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                <h3 className="text-lg font-bold text-white flex items-center gap-2">
                                    ...
                                    Ahli Keluarga Terhubung
                                </h3>
                                <p className="text-xs text-slate-400">Pilihan akses dan kawalan pemantauan.</p>
                            </div>
                            <button
                            ...
                            >
                                <Plus className="w-4 h-4" /> Jemput Ahli Baru
                            </button>
                        </div>
                            ...
                            {connectedCaregivers.map((cg) => (
                                <div key={cg.id} className="p-4 rounded-xl bg-slate-800/60 border border-slate-700/60 flex items-center justify-between gap-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-2">
                                                ...
                                                {cg.isPrimary && <Badge variant="teal">Primary ICE</Badge>}
                                            </div>
                                    ...
                                    <div className="text-right">
                                        ...
                                        <span className="text-[10px] text-slate-500">Ahli sejak {cg.joinedDate}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    <div className="space-y-4 relative before:absolute before:inset-0 before:left-3 before:w-0.5 before:bg-slate-800">
                        {sharedLog.map((log) => (
                            <div key={log.id} className="relative pl-7 space-y-1">
                                ...
                                <p className="text-[11px] text-slate-400">Oleh: <span className="text-slate-300 font-semibold">{log.actor}</span></p>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <Modal isOpen={isAddMemberModal} onClose={() => setIsAddMemberModal(false)} title="Jemput Ahli Keluarga / Caregiver">
                <div className="space-y-4 text-center py-2">
                    ...
                    <h4 className="text-base font-bold text-white">Imbas atau Kongsi Kod Sambungan</h4>
                    <p className="text-xs text-slate-300 max-w-sm mx-auto">
                        Minta anak atau ahli keluarga memuat turun MediSync dan memasukkan kod sambungan di bawah untuk menyambung profil kesihatan.
                    </p>
                    ...
                    >
                        {copied ? 'Kod Berjaya Disalin!' : 'Salin Kod & Hantar Ke WhatsApp'}
                    </button>
                </div>
            </Modal>
        </AppLayout>
