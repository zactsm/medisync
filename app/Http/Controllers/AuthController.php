...
class AuthController extends Controller
{
    ...
    public function sync(Request $request)
    {
        ...
        abort_unless($phone, 422, 'A phone number is required');
        $user = User::firstOrCreate(['phone' => $phone], ['name' => $remote['user_metadata']['full_name'] ?? 'MediSync User', 'email' => $remote['email'] ?? null, 'password' => Hash::make(Str::random(48)), 'role' => 'patient', 'ice_code' => 'ICE-'.strtoupper(Str::random(8)), 'caregiver_sync_code' => 'MS-'.strtoupper(Str::random(8))]);
